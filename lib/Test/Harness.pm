...
$ENV{REGRESSION_LEVEL}=1;  #testing for this variable is strongly discouraged!
...
sub runtests {
    my(@tests) = @_;
    local($|) = 1;
    my($test,$te,$ok,$next,$max,$pct,$totok,@failed,%failedtests,$bonusok);
    my $totmax = 0;
    my $files = 0;
    my $bad = 0;
    my $good = 0;
	...
	$te = $test;
	chop($te);
	$ENV{REGRESSION_TEST}=$te;  #using this variable is strongly discouraged!
	...
	if ($wstatus) {
	    ...
	    if ($max) {
		my $skip_i = (!$skipped? '': "$skipped subtest".
			      ($skipped>1? 's':'')." skipped on this platform");
		my $fail_i = (!$failok? '': "$failok subtest".
			      ($failok>1?'s':'')." failed as expected");
		if ($skip_i or $fail_i) {
		    print "ok (".join(", ", $skip_i, $fail_i).")\n";
		} else {
		    print "ok\n";
		}
	    } else {
    ...
    my $bonus = ($bonusok? " ($bonusok subtest".($bonusok>1?'s':'').
		 " unexpectedly succeeded)":'');
    if ($bad == 0 && $totmax) {
	print "All tests successful$bonus.\n";
    } elsif ($total==0){
	...
	my $subpct = sprintf " %d/%d subtests failed, %.2f%% okay$bonus.",
